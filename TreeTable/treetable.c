 ...
 * Returns -1 if the subtree is unbalanced (height difference > 1).
 * The sentinel node has height 0.
 */
static int tree_height(TreeTable *t, RBNode *n) {
    if (n == t->sentinel)
        return 0;

    int left_h = tree_height(t, n->left);
    int right_h = tree_height(t, n->right);
...
/*
 * Returns 1 if the tree table t is key balanced, 0 otherwise.
 * A tree is balanced if for any node, the height of the left and
 * right subtrees does not differ by more than 1.
 */
int balanced(TreeTable *t) {
    return tree_height(t, t->root) != -1;
}

/*
 * Recursive helper that checks the BST ordering property.
 * min and max are bounds that the current node's key must fall within.
 * NULL means "no bound on this side".
 */
static int sorted_helper(TreeTable *t, RBNode *n, void *min, void *max) {
    if (n == t->sentinel)
        return 1;

    if (min != NULL && t->cmp(n->key, min) <= 0)
        return 0;

    if (max != NULL && t->cmp(n->key, max) >= 0)
        return 0;

    return sorted_helper(t, n->left,  min,    n->key)
        && sorted_helper(t, n->right, n->key, max);
}

/*
 * Returns 1 if the tree table t is key sorted, 0 otherwise.
 * A tree is sorted if for any node, its key is greater than all
 * keys in its left subtree and smaller than all keys in its right subtree.
 */
int sorted(TreeTable *t) {
    return sorted_helper(t, t->root, NULL, NULL);
}
